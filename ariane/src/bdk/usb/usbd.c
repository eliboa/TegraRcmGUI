...
{
    //gfx_printf("_usbd_get_ep_status\n");
	bool status;
	u32 reg_val;
	u32 reg_mask;
	...
	{
		gfx_printf("token %d \n", usbdaemon->qhs[endpoint].token);
		if (usbdaemon->qhs[endpoint].token & USB_QHD_TOKEN_HALTED)
		{
			gfx_printf("USB_QHD_TOKEN_HALTED\n");
		}
		if (usbdaemon->qhs[endpoint].token & USB_QHD_TOKEN_BUFFER_ERROR)
		{
			gfx_printf("USB_QHD_TOKEN_BUFFER_ERROR\n");
		}
		if (usbdaemon->qhs[endpoint].token & USB_QHD_TOKEN_XFER_ERROR)
		{
			gfx_printf("USB_QHD_TOKEN_XFER_ERROR\n");
		}
...
out:
		...
		{
			usb_ep_status_t status = _usbd_get_ep_status(endpoint);
			if (status != USB_EP_STATUS_IDLE)
			{
                //gfx_printf("status is %d\n", status);
				res = 26;
			}
		}
	}
...
static int _usbd_handle_set_request(int *ep_stall)
{
	...
	{
        //gfx_printf("USB_REQUEST_SET_CONFIGURATION\n");
		ret = _usbd_ep_ack(USB_EP_CTRL_IN);
		if (!ret)
		{
