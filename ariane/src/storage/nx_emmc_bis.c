...
static int nx_emmc_bis_read_block(u32 sector, u32 count, void *buff)
{
    ...
    int res = emu_cfg.enabled && !h_cfg.emummc_force_disable ? emummc_part_read(&emmc_storage, system_part, sector, count, buff) : nx_emmc_part_read(&emmc_storage, system_part, sector, count, buff);
    if (res)
	{
