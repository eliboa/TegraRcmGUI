...
int emummc_storage_init_mmc(sdmmc_storage_t *storage, sdmmc_t *sdmmc)
{
	...
	if (!emu_cfg.sector)
	{
		...
		{
            gfx_printf("Failed to open eMMC folder.");
			goto out;
		}
		...
		if (f_stat(emu_cfg.emummc_file_based_path, &fno))
		{
            gfx_printf("Failed to open emuMMC rawnand.");
			goto out;
		}
...
int emummc_part_read(sdmmc_storage_t *storage, emmc_part_t *part, u32 sector_off, u32 num_sectors, void *buf)
{
